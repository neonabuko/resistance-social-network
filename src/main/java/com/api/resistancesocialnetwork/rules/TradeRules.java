...
public class TradeRules {
    public void handle(Rebel sourceRebel, Rebel targetRebel,
                       Integer sourceItemId, Integer targetItemId) throws TradeFailureException {

        assert_traders_are_allies(sourceRebel, targetRebel);

        assert_inventory_has_item(sourceRebel.getInventory(), sourceItemId);

        assert_inventory_has_item(targetRebel.getInventory(), targetItemId);

        Integer sourceItemPrice = sourceRebel.getInventory().findItemBy(sourceItemId).get().getPrice();
        Integer targetItemPrice = targetRebel.getInventory().findItemBy(targetItemId).get().getPrice();
